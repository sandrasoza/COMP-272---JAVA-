 ...
 *        V remove(K)               - Removes the entry for the specified key and returns the removed value,
 *                                      or null if the key is not found
 *  boolean remove(K, V)            - Removes the entry for the specified key only if it is currently
 *                                    mapped to the specified value
 *        V replace(K, V)           - Replaces the entry for the specified key only if it is currently
 *                                    mapped to some value
 *  boolean replace(K, V, V)        - Replaces the entry for the specified key only if currently mapped
 ...
 *      - values()
 *
 * Also, null keys are not supported in this implementation.
 * However,  Java-HashMap behavior allows one null key.
 ****************************************/
...
class HashNode<K,V> {
    ...
    public HashNode(K key, V value) {
        // Assumption: null keys are not supported in this implementation.
        this.key = key;
        this.value = value;
        this.next = null;
    }
...
class myHashMap<K,V> {
     ...
     * The method will probe into the bucket lists. If the bucket
     ...
     *
     * @return value - return the value for the <key,value>
    ...
    public V put(K key, V value) {
         ...
         * then replace its value with new one and return the old one ,
     ...
     *
     *  @return boolean  - True if found and replaced,
     *                     False otherwise.
     */
    ...
    public boolean containsValue(V val) {
        ...
        for (HashNode<K, V> headNode : bucket) {
            while (headNode != null) {
                if ( Objects.equals(headNode.value, val) )
                    return true;
                headNode = headNode.next;
            }
        }
