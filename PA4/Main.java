...
public class Main {
    public static void main(String[] args) {
        ...
        /*******************************************************************
         *
         *        Tests for the myHashMap remove(K, V) method
         *
         ******************************************************************/

        // should return false because the key does not exist
        if (!removeErrorFlag && hashmap.remove("DOES_NOT_EXIST", 100)) {
            System.out.println("Error 7: remove(K,V) failed");
            removeErrorFlag = true;
        }

        // should return false because value does not match
        if (!removeErrorFlag && hashmap.remove("Key_6", 999)) {
            System.out.println("Error 8: remove(K,V) failed");
            removeErrorFlag = true;
        }

        // should return true because key and value match
        if (!removeErrorFlag && !hashmap.remove("Key_6", 6)) {
            System.out.println("Error 9: remove(K,V) failed");
            removeErrorFlag = true;
        }

        // verify key and value are actually removed
        if (!removeErrorFlag && hashmap.containsKey("Key_6")) {
            System.out.println("Error 10: remove(K,V) failed");
            removeErrorFlag = true;
        }

        if (!removeErrorFlag && hashmap.containsValue(6)) {
            System.out.println("Error 11: remove(K,V) failed");
            removeErrorFlag = true;
        }
        ...
        if ( ! getAverageErrorFlag && Math.abs(hp.getAverage(hash_map, arr2) - 36.6) > 1e-9 ) {
